...
class Geometry:
    ...
    #                             OUTPUT PARSER                               #
    ###########################################################################
    def save_to_file(self, filename, mode="w", **kwargs):
        """
        Save geometry to a file.

        Parameters:
            filename (str): The output file path.
            mode (str): File write mode ('w' = write, 'a' = append, etc.).
            **kwargs: Passed to get_text().
        """
        ...
        # Enforce Linux line endings regardless of platform
    ...
    def get_from_ase_atoms_object(self, atoms) -> None:
        ...
        # Conditionally assign energy
        self.energy = atoms.info.get("energy", None)

        # Conditionally assign forces
        self.forces = atoms.arrays.get("forces", None)

    def get_as_ase(self, energy_key="energy", froces_key="forces") -> None:
        """
    ...
    def add_multipoles(
        self, multipoles: Union[List[float], List[list]]
    ) -> None:
        """
        ...
        for s in self.species:
            try:
                if "_" in s:
                    ...
                    curr_species = s
                electrons.append(
                    self.periodic_table.get_atomic_number(curr_species)
                )
    ...
    def get_spglib_cell(
        ...
        for atom_name in self.species:
            atom_number.append(
                self.periodic_table.get_atomic_number(atom_name)
            )
    ...
    def get_cropping_indices(
        ...
        if auto_margin:
            margin = max(
                [
                    self.periodic_table.get_covalent_radius(s)
                    for s in self.species
                ]
            )
    ...
    def get_indices_of_all_atoms(self, species=None):
        if species is None:
            ...
            return [
                i for i in range(self.n_atoms) if self.species[i] == species
            ]
    ...
    def get_transformation_indices(
        ...
        Z_values_2 = np.array(
            [
                self.periodic_table.get_atomic_number(s)
    ...
    def get_primitive_slab(self, surface, threshold=1e-6):
                ...
                for k in range(-shell, shell + 1):
                    for l in range(-shell, shell + 1):
                        ...
                        for new_species, coord in zip(
                            atomic_numbers, scaled_positions
                        ):
        ...
        species = [
            self.periodic_table.get_atomic_number(s) for s in slab.species
        ]
    ...
    def get_split_into_molecules(self, threshold) -> list:
        ...
        def scan_line(line_index, matrix, already_scanned_lines_indices):
            ...
            links = [
                l for l in links if l not in already_scanned_lines_indices
            ]
    ...
    def visualise(
        ...
        n_atoms = len(species)
        circlesize = [
            self.periodic_table.get_covalent_radius(s) * atom_scale
            for s in species
        ]
...
class AimsGeometry(Geometry):
    ...
    def get_text(self, is_fractional=None):
        ...
        if hasattr(self, "vacuum_level") and (self.vacuum_level is not None):
            text += (
                "set_vacuum_level {: 15.10f}".format(self.vacuum_level) + "\n"
            )
...
class XYZGeometry(Geometry):
    ...
    def get_text(self, comment=None, energy_key="energy", froces_key="forces"):
        """
        Generate an XYZ-format string for the geometry, including optional
        energy and forces.

        Parameters:
            comment (str or None): Optional comment. If None, a default will be
                                   generated based on energy availability.

        Returns:
            str: Formatted XYZ text.
        """
        text = f"{self.n_atoms}\n"

        # Construct comment line
        if comment is None:
            comment_parts = ["XYZ file written by Geometry.py"]
            if self.energy is not None:
                comment_parts.append(f"energy={self.energy:.8f}")
            if self.forces is not None:
        ...
        for index in range(self.n_atoms):
            ...
            line = f"{element}    {x:-4.8f}    {y:-4.8f}    {z:-4.8f}"

            if self.forces is not None:
                F_x, F_y, F_z = self.forces[index]
                line += f"    {F_x:-4.8f}    {F_y:-4.8f}    {F_z:-4.8f}"
