...
class TestMathUtils:
    ...
    def test_get_rotation_matrix_around_axis(self):
        axis = np.array([0, 0, 1])
        phi = np.pi / 2
    ...
    def test_get_mirror_matrix(self):
    ...
    def test_get_angle_between_vectors(self):
        v1 = np.array([1, 0, 0])
        v2 = np.array([0, 1, 0])
        angle = get_angle_between_vectors(v1, v2)
        assert np.isclose(angle, 0)

    def test_get_fractional_coords(self):
        lattice = np.eye(3)
    ...
    def test_get_triple_product(self):
        a = np.array([1, 0, 0])
        b = np.array([0, 1, 0])
        c = np.array([0, 0, 1])
    ...
    def test_get_cross_correlation_function(self):
    ...
    def test_get_fourier_transform(self):
